...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        transitions_per_parse = model.predict(unfinished_parses[:batch_size])
        for unfinished_parse, transition_pp in zip(
            unfinished_parses[:batch_size], transitions_per_parse
        ):  
            unfinished_parse.parse_step(transition_pp)
        unfinished_parses = [
            parse
            for parse in unfinished_parses
            if (len(parse.stack) > 1) or (parse.buffer)
        ]
