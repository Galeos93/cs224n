...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.empty(embedding_size * n_features, hidden_size))
    ...
    def embedding_lookup(self, w):
        ...
        ###     Index select: https://pytorch.org/docs/stable/torch.html#torch.indctex_select
        ###     Gather: https://pytorch.org/docs/stable/torch.html#torch.gather
        ###     View: https://pytorch.org/docs/stable/tensors.html#torch.Tensor.view
        _, embed_size = self.embeddings.size()
        batch_size, features_size = w.size()
        x = torch.index_select(self.embeddings, 0, w.view(-1))
        x = x.view(batch_size, features_size * embed_size)
    ...
    def forward(self, w):
        ...
        hidden_feats = torch.matmul(embeddings, self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        hidden_feats = torch.relu(hidden_feats)
        logits = torch.matmul(hidden_feats, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
