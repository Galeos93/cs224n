...
def question_1g_sanity_check():
    ...
    x = torch.Tensor(np.ones((sentence_length, batch_size, char_embedding_size, max_word_length)))
    ...
    output = cnn.forward(x)
    expected_output = torch.Tensor(np.ones((sentence_length, batch_size, word_embedding_size)) * 30)
...
def main():
    ...
    decoder = CharDecoder(
        hidden_size=HIDDEN_SIZE, char_embedding_size=EMBED_SIZE, target_vocab=char_vocab
    )
