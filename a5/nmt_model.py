...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        ...
        self.h_projection = nn.Linear(self.hidden_size*2, self.hidden_size, bias=False) # The final vector of hidden states
        self.c_projection = nn.Linear(self.hidden_size*2, self.hidden_size, bias=False) # The final vector of cell states
        ...
        self.target_vocab_projection = nn.Linear(self.hidden_size, len(self.vocab.tgt), bias=False) # We don't need embedding since we don't need to embed the output on low dimensions?
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device) # Tensor: (tgt_len, b, w_len)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens, lengths = pad_packed_sequence(enc_hiddens) # L, B, Hx2
        enc_hiddens = torch.transpose(enc_hiddens, 0, 1) # B, L, Hx2
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, 2)), 2) # (b, src_len, h), (b, h, 1) = (b, src_len, 1)
        ...
        a_t = torch.squeeze(torch.bmm(torch.unsqueeze(alpha_t, 1), enc_hiddens), 1) # (b, 1, src_len) x (b, src_len, h) = (b, 1, h)
